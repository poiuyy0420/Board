...
public class BoardDAO {
	...
	//이 곳에 게시판 작업의 기능 하나하나를 메서드로 추가 예정
	public void insertArticle(BoardVO article) { //실제 데이터베이스에 데이터를 넣어주는 메소드
	...
	public int getArticleCount() { //첫번째 전체 글의 개수를 가져올 메서드
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		int x = 0;
	...
	public List<BoardVO> getArticles (int start, int end){ //board table에서 데이터를 가져올 메소드
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		...
		try {
			...
			if(rs.next()) {
				...
				do {
					...
					article.setDepth(rs.getInt("step"));
	...
	//글수정 - 조회수를 증가시키는 부분을 제외하고 단순히 num에 해당하는 게시물만 가져오는 메소드
	...
	//글삭제 - 데이터베이스에서 비밀번호를 비교하여 실제로 삭제를 수행해 줄 메서드
